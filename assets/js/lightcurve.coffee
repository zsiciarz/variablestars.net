...
class LightCurve
    ...
    setGeometry: ->
        @margin =
            ...
            bottom: 40
            left: 50
        @width = d3.select(@selector).node().offsetWidth - 40
        @height = 0.3 * @width
...
selector = '.lightcurve'
epoch = +d3.select('.catalog-data .epoch').text()
period = +d3.select('.catalog-data .period').text()
lc = new LightCurve(selector, epoch, period)
csvUrl = d3.select(selector).node().dataset.csvSource
if csvUrl
    isPhaseChart = false
    d3.csv(
        csvUrl,
